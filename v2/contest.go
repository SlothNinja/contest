...
func New(id int64, pk *datastore.Key, gid int64, t game.Type, r, rd, outcome float64) *Contest {
	return &Contest{
...
func (client *Client) UnappliedFor(c *gin.Context, ukey *datastore.Key, t game.Type) ([]*Contest, error) {
...
type ContestMap map[game.Type][]*Contest
...
func (client *Client) Unapplied(c *gin.Context, ukey *datastore.Key) (ContestMap, error) {
	...
	cm := make(ContestMap, len(game.Types()))
	for _, c := range cs {
		c.Applied = true
